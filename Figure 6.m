...
tic
for p=2:200
    p1=1-lambda(X(p-1))*dtt;   %simulation of markov chain
    k2=rand;
    if k2<=p1
        X(p)=X(p-1);
    elseif X(p-1)==1
...
for p=1:200
    ...
    A=st/dx;
    if X(p)==1
        C(p)=((A-floor(A))*u(N,ceil(A),1))+((floor(A+1)-A)*u(N,floor(A),1));
        
    elseif X(p)==2
        C(p)=((A-floor(A))*u(N,ceil(A),2))+((floor(A+1)-A)*u(N,floor(A),2));
        
    else
        C(p)=((A-floor(A))*u(N,ceil(A),3))+((floor(A+1)-A)*u(N,floor(A),3));
        
    end
    
      %Implied volatility  
    IV(p)=blsimpv(st,st,0,(N*dt),C(p));
end
xlswrite('IV1.xlsx',transpose(S),'A2:A201')
xlswrite('IV1.xlsx',transpose(C),'B2:B201')
xlswrite('IV1.xlsx',transpose(X),'C2:C201')
xlswrite('IV1.xlsx',transpose(IV),'D2:D201')
xlswrite('IV1.xlsx',transpose(SIG(X)),'E2:E201')
